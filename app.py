...
from audio_helper import record_and_transcribe, speak_text
...
if st.session_state.stage == "home":
    ...
    # Cleaned grid columns wrapper for parameter settings
...
# ── INTERVIEW PAGE ────────────────────────────────────────────────────────────
    ...
    with col_b:
        if st.button("🎤 Record my answer", type="primary", use_container_width=True, disabled=st.session_state.answered):
            with st.spinner("Listening… speak your answer now."):
            ...
            if user_answer:
                ...
                st.error("No audio detected. Check your microphone and try again.")
